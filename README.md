# 👋 Olá, eu sou a Roberta Rocha

**Analista de Sistemas | 15+ anos em Telecom | Python | IA | AWS | Cibersegurança**

Sou uma profissional apaixonada por tecnologia, com sólida experiência na sustentação e evolução de sistemas críticos em empresas de telecomunicações. Após 15 anos trabalhando em plataformas como **SIEBEL**, **Clarify**, **Microsiga** e ambientes corporativos complexos, iniciei uma jornada em um novo universo: **Inteligência Artificial, Cloud Computing (AWS) e Cibersegurança**.

Atualmente estudo **IA**, **Python**, **Arquiteturas Serverless**, **AWS** e **segurança ofensiva**, combinando minha bagagem técnica com novos desafios do mundo cloud.

---

## 🚀 Sobre mim

- 🔧 **15 anos de experiência em TI**, atuando como Analista Pleno em grandes empresas como **Oi S/A** e **Accenture**  
- 📡 Especialista em sustentação de sistemas de **Automação de Vendas (SFA/SIEBEL B2B)**  
- 🧩 Forte experiência com **sustentação, investigação, testes e análise de sistemas corporativos**  
- 🧠 Em formação contínua em **IA, Machine Learning, Python Avançado e AWS**  
- 🔐 Estudando **Cibersegurança (Hackers do Bem)**  
- 🌎 Objetivo atual: unir experiência corporativa com soluções modernas em **Cloud, Serverless, IA e DevOps**

---

## 📘 Formações e Estudos Atuais

- 🎓 **Engenharia de Inteligência Artificial** – Data Science Academy (conclusão: 05/2026)  
- 🐍 **Python Impressionador** – Hashtag Treinamentos (07/2025)  
- 🔐 **Cibersegurança – Hackers do Bem** (10/2025)  
- ☕ **Java OO** – Caelum (40h)  
- 🏛️ Experiência com **Oracle, PL/SQL, SQL e Delphi** em ambiente TCU

---

## 💼 Experiência Profissional (Resumo)

### 🏢 Oi S/A (2018–2022)  
Analista Pleno – Sustentação do **SFA SIEBEL8 (Fibra)** e sistemas de Automação de Vendas B2B.

### 🏢 Globalweb (2014–2018)  
Sustentação de **SIEBEL8**, **Clarify**, **Geco**, **Transact** e **Microsiga**.

### 🏢 Accenture (2009–2014)  
Atuação contínua em  
- SFAWEB  
- Portabilidade  
- Geco (Gestão de Contratos)  
- DataQuality  
- ADVPL (Microsiga)  
- Execução de testes e sustentação em sistemas corporativos

### 🏛️ TCU – Tribunal de Contas da União (2008–2009)  
Desenvolvimento em **Delphi**, **SQL**, **Oracle** e **PL/SQL**.

---

## 📌 Tecno­logias & Habilidades

**Linguagens:** Python, SQL, PL/SQL, Java (básico), Delphi  
**Cloud:** AWS (Lambda, API Gateway, DynamoDB)

